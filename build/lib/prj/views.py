...
def find(request):
	...
	if request.method == "POST":
		if request.POST.get('find'):
			topics = request.db['topic'].find( {"title": {'$regex': request.POST.get('find'), 
														 '$options': 'i'}});	
		else:
			topics = False	
...
def last_five(request):
	'''
	Get last 5 topics inserted. 
	flag_five - set h1 in Home.html if last_five called.
	(write less)
	'''
	flag_five = True
	topics = request.db['topic'].find().sort([("$natural", -1),("topic_date", -1)]).limit(5)
	return render_to_response('templates/home.html',
							  {'topics':topics,
							   "flag_five":flag_five,
							   'count':count(request)},
